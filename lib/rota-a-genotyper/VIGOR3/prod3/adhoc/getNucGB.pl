...
for my $acc ( @ARGV ) {
    print "\nfetching $acc\n";
    $params{email} = "jhoover@jcvi.org";
    $params{db} = "nuccore";
    $params{tool} = "ebot";
    $params{term} = $acc . "[accn]";
    %params = esearch(%params);
    
    $params{retmode} = "text";
    $params{outfile} = $acc . ".gbk";
    $params{rettype} = "gb";
    efetch_batch(%params);
    
    #$params{linkname} = "protein_nuccore";
    #%params = elink_batch_to('nuccore', %params);
    
    #$params{retmode} = "text";
    #$params{outfile} = $protacc . ".fna";
